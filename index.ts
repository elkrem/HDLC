...
class HDLC {
    ...
    pendingFrame:any = {};
    byteSendingFunction = function (byte) {};
    public eventEmitter = new EventEmitter();


    public init(_byteSendingFunction) {
        this.byteSendingFunction = _byteSendingFunction;
        ...
        for (var i = 0; i < bytes.length; i++) {
            ...
            if (data == FRAME_BOUNDARY_OCTET) {
                ...
                /* If a valid frame is detected */
                    ...
                    (this.pendingFrame.frameChecksum == ((this.pendingFrame.receivedFrameBuffer[this.pendingFrame.framePosition - 1] << 8) | (this.pendingFrame.receivedFrameBuffer[this.pendingFrame.framePosition - 2] & 0xff)))) // (msb << 8 ) | (lsb & 0xff)
                {
